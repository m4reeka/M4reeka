...
# AUTO_BLEND_INFLUENCES_PATCH: choose 4 or 8 bone influences on export based on mesh data
def _auto_choose_blend_influences(mesh: bpy.types.Mesh, cap: int = 8) -> int:
    """Return 8 if any vertex uses >4 nonzero weights, else 4."""
    max_inf = 0
        ...
        for g in v.groups:
            if g.weight > 0.0:
                ...
                if max_inf >= cap:
                    return cap
        if max_inf > 4:
            # keep scanning a bit but we already know we need 8
            return cap
    return 4 if max_inf <= 4 else cap

def _apply_blend_layout(influences: int):
    """Patch DataModelWWMI Blend layout in-memory (4 or 8)."""
    influences = 8 if influences > 4 else 4
    DataModelWWMI.layouts['Blend'] = BufferLayout([
        BufferSemantic(AbstractSemantic(Semantic.Blendindices, 0), DXGIFormat.R8_UINT, stride=influences),
        BufferSemantic(AbstractSemantic(Semantic.Blendweight, 0), DXGIFormat.R8_UINT, stride=influences),
    ])
